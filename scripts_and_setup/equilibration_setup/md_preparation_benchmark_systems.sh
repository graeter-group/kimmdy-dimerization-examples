...
for file in *.pdb; do
	...
	for (( run=1; run<=run_num; run++ )); do
		...
		sed -i "s|^#SBATCH --job-name=.*|#SBATCH --job-name=${sequence_name}_R${run}_NPT|" resubmit_npt.sh
        	...
        	sed -i "s|^#SBATCH --job-name=.*|#SBATCH --job-name=${sequence_name}_R${run}_MD|" resubmit_md.sh
	
		# Create gromacs files
		echo -e "1\n1\n" | GMXLIB="$(pwd)" gmx pdb2gmx -f "${sequence_name}.pdb" -o "${sequence_name}.gro" -p "${sequence_name}.top" -i "${sequence_name}.itp" 

		# Adjust box size (HARDCODED, depends on system see SI)
		gmx editconf -f "${sequence_name}.gro" -o "${sequence_name}_newbox.gro" -c -box "10" "10" "10"
